...
const WithInfo = ({ children }) => {
  ...
  const ageOptions = [ { type: "65 yaş üzeri", priority: 1 }, { type: "50-65 yaş arası", priority: 2 }, { type: "20-50 yaş arası", priority: 3 }, { type: "20 yaş altı", priority: 4 }]
  const occupationOptions = [{ type:"Sağlık çalışanı", priority: 1 }, { type:"Kalabalık ortamlarda çalışan", priority: 2 }, { type:"Diğer", priority: 3 }, { type:"İşsiz", priority: 4 } ]
  ...
  const changeAge = ( age ) => {
   ...
   const decideGroup = () => {
      if(age !== -1 && occupation !== -1){
        const selectedOcc = occupationOptions.find(option => option.type === occupation);
        const selectedAge = ageOptions.find(option => option.type === age);
        const diseasePriority = !hasDisease ? 4 : (selectedAge.priority === 3 ? 3 : 2);
        const decisionArray = [selectedAge.priority, selectedOcc.priority, (handicapped ? 1 : 4), diseasePriority];
